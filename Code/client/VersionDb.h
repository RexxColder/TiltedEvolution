...
public:
    ...
    bool Load(const std::filesystem::path& acGamePath, int major, int minor, int revision, int build)
    {
...
#if TP_SKYRIM
        fullPath = acGamePath / "Data" / "SKSE" / "Plugins" / fileName;
#elif TP_FALLOUT4
        fullPath = acGamePath / "Data" / "F4SE" / "Plugins" / fileName;
#else
        fullPath = acGamePath / fileName;
#endif
        std::ifstream file(fullPath, std::ios::binary);
        if (!file.good())
        {
#if TP_FALLOUT4
            // Fallback for FO4: try "version-X-X-X-X.bin" (Address Library dump format)
            _snprintf_s(fileName, 256, "version-%d-%d-%d-%d.bin", major, minor, revision, build);
            fullPath = acGamePath / "Data" / "F4SE" / "Plugins" / fileName;
        ...
        if (format == 2)
        {
            // Standard Address Library format (Format 2)
            for (int i = 0; i < 4; i++)
            ...
            if (tnLen > 0)
            {
                std::vector<char> tnbuf(tnLen + 1, '\0');
                file.read(tnbuf.data(), tnLen);
                _moduleName.assign(tnbuf.data(), tnLen);
            }
        ...
        {
            // Handling flat dump format (count + padding + [ID, Offset] pairs)
            // format here is actually the entry count (e.g. 1.5M)
            if (format <= 0 || format > 0x1000000)
                return false;

            int addrCount = format;
            read<int>(file); // Skip padding / version internal field
            ...
            for (int i = 0; i < addrCount; i++)
            {
                unsigned long long q1 = read<unsigned long long>(file);
                unsigned long long q2 = read<unsigned long long>(file);
                _data[q1] = q2;
                _rdata[q2] = q1;
            }
        }
