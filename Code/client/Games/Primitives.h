...
template <class T> void GameArray<T>::Resize(size_t aLength) noexcept
{
    if (capacity >= aLength)
    {
...
template <class T> struct BSTSmallArray
{
    int32_t capacity; // 0
    T** data;         // 8
    uint32_t size;    // 10
...
template <class T> struct GameValueList
{
    ...
    inline bool Empty() const noexcept { return !entry.next && !static_cast<bool>(entry.data); }
    ...
    Iterator begin() { return Empty() ? end() : Iterator(&entry); }
...
struct NiPoint3 : glm::vec3
{
    ...
    NiPoint3(const glm::vec3& acRhs)
        : glm::vec3(acRhs)
    {
    }
    NiPoint3(float aX, float aY, float aZ)
        : glm::vec3(aX, aY, aZ)
    {
    }
...
template <class T> struct BSScopedLock
{
    BSScopedLock(T& aLock)
        : m_lock(aLock)
    {
        m_lock.Lock();
    }
...
template <class T> struct GamePtr
{
    GamePtr()
        : m_pPointer(nullptr)
    {
