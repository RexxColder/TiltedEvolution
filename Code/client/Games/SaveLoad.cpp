...
#elif TP_FALLOUT4
    // F4SE equivalent: BGSSaveFormBuffer::ctor
    // Find with: F4SE Address Library — search for "BGSSaveFormBuffer" constructor.
    POINTER_FALLOUT4(CtorT, ctor, 40755); // Official FO4 AL ID
#endif
...
#elif TP_FALLOUT4
    // F4SE equivalent: BGSLoadFormBuffer::ctor
    // Find with: F4SE Address Library — search for "BGSLoadFormBuffer" constructor.
    POINTER_FALLOUT4(CtorT, ctor, 40756); // Official FO4 AL ID
#endif
...
static TiltedPhoques::Initializer s_saveLoadHooks(
    []()
    {
...
#elif TP_FALLOUT4
        // F4SE equivalents to find in the Address Library:
        //   s_readFormId   -> BGSLoadFormBuffer::ReadFormId  (loads a FormID from save data)
        //   s_writeFormId  -> BGSSaveFormBuffer::WriteFormId (writes a TESForm* as FormID)
        //   s_writeId      -> BGSSaveFormBuffer::WriteId     (writes a raw uint64 FormID)
        POINTER_FALLOUT4(TBGSLoadFormBuffer_ReadFormId, s_readFormId, 40781);
        POINTER_FALLOUT4(TBGSSaveFormBuffer_WriteFormId, s_writeFormId, 33966);
        POINTER_FALLOUT4(TBGSSaveFormBuffer_WriteId, s_writeId, 33967);
#endif
        ...
        // Only install hooks if the target functions were resolved (non-null).
        // FO4 pointers are 0 until the Address Library IDs are filled in.
        if (RealBGSLoadFormBuffer_ReadFormId)
            TP_HOOK(&RealBGSLoadFormBuffer_ReadFormId, BGSLoadFormBuffer_LoadFormId);
        if (RealBGSSaveFormBuffer_WriteFormId)
            TP_HOOK(&RealBGSSaveFormBuffer_WriteFormId, BGSSaveFormBuffer_WriteFormId);
        if (RealBGSSaveFormBuffer_WriteId)
